...
for teste in ["teste1_1.msh", "teste1_2.msh", "teste1_3.msh"]:
    m = ns.Mesh(file=path + teste, verbose=True)
    ...
    vc, ic, bc = m.run(cuda=False, boundary=bound, alpha=alpha)
    bench['cpu']['iter'].append(ic)
    bench['cpu']['integration'].append(bc[0])
    bench['cpu']['solve'].append(bc[1])

    vg, ig, bg = m.run(cuda=True, boundary=bound, alpha=alpha)
